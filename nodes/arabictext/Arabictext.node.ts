...
import gm from 'gm'; // gm (GraphicsMagick) for image manipulation
import { file } from 'tmp-promise'; // For temporary file creation
import { parse as pathParse } from 'path'; // Path parsing utilities
import { writeFile as fsWriteFile } from 'fs'; // File system write utility
import { promisify } from 'util'; // Promisify utility for async file writes

const fsWriteFileAsync = promisify(fsWriteFile);
import getSystemFonts from 'get-system-fonts'; // Get system fonts utility
			...
			try {
				...
				for (let i = 0; i < operations.length; i++) {
					...
					if (operationData.operation === 'blur') {
						...
						const renderText: string = lines.join('\n');

						// Determine the font to use (default or passed font from options)
						const font: string | undefined = (typeof options.font === 'string' ? options.font : (typeof operationData.font === 'string' ? operationData.font : undefined));

						// Ensure Pango is used for Arabic and complex text rendering
						if (font && font !== 'default') {
							gmInstance = gmInstance!.font(`Pango:${font}`); // Use Pango for complex scripts like Arabic
						}

						// Apply font color, size, region, and gravity for text drawing
						gmInstance = gmInstance!
							...
							.drawText(
								...
								operationData.positionY as number,
								renderText,
								operationData.gravity as string
							);
